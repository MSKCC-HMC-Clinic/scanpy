...
def gsea(
    ...
    input_gene_ranking_file
        ...
        second column (numberic type) is their respective rankings by weight, with NO header or index column
    ...
    -------
    Inputs:
        For hallmark_gene_sets, user must either provide a .gmt file
    ...
    Outputs:
        ...
        based on the type of GSEA implementation chosen
    ...
    Implementation:
        ...
        GSEApy has issues with closing its own log file, where there may be
        conflicts with writing to the same directory when running the "gseapy" option

    Example
    -------
    # more examples can be found in tests/external/test_gsea.py
    >>> gsea_df = scanpy.external.tl.gsea(input_gene_ranking_file='./_data/gsea_data.csv', hallmark_gene_sets_file='./_data/KEGG_2016.gmt', type='gseapy')
    >>> fgsea_df = scanpy.external.tl.gsea(input_gene_ranking_file='./_data/gsea_data.gsea_data.rnk', hallmark_gene_sets_file='./_data/KEGG_2016.gmt', type='fgsea', rscript_path=rscript_path)
    ...
    if type == 'gseapy':
        ...
        if Path(input_gene_ranking_file).suffix == '.rnk':
            ...
            try:
                ...
                raise AssertionError('.csv format for input_gene_ranking_file is string type for gene list (first column) and numeric type for gene ranking values (second column) with NO header or index column.')
